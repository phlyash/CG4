...
import com.cgvsu.render_engine.rasterization.RenderingPreprocessing;

import java.util.ArrayList;
import java.util.List;

public class AffineModelTransform {
    ...
    public AffineModelTransform(Model model) {
        if (model == null) {
    ...
    private List<Polygon> triangulateAndRecalculateNormals(Model model) {
        ...
        for (Polygon polygon : model.polygons) {
            triangulatedPolygons.addAll(RenderingPreprocessing.triangulation(polygon));
        }

        RenderingPreprocessing.recalculateNormals(model);
